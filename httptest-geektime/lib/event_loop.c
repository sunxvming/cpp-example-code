...
int event_loop_do_channel_event(struct event_loop *eventLoop, int fd, struct channel *channel1, int type) {
    ...
    if (!isInSameThread(eventLoop)) {
        // 父线程中，来新连接时，让 sub-reactor 线程从 event_dispatcher 的 dispatch 上返回，
        // 因为之前已经把channel加到子的loop_event中了，唤醒后执行event_loop_handle_pending_channel
...
//这个函数就是简单的从 socketPair[1] 描述字上读取了一个字符而已，除此之外，
//它什么也没干。它的主要作用就是让子线程从 dispatch 的阻塞中苏醒
